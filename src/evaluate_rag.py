...
RESULTS_FILE = PROJECT_ROOT / "results" / "rag_evaluation_results.csv"
SUMMARY_FILE = PROJECT_ROOT / "results" / "rag_evaluation_summary.json"
...
def evaluate_case(
    ...
    request = RAGRequest(
        ...
        context={"behavior": case.context},
        escalation_reason="LOW_CONFIDENCE_AND_DRIFT",
        request_id=case.case_id,
    )
    result = pipeline.run(request)
    ranked_ids = [item.evidence_id for item in result.retrieved.evidence]
    expected_rank = ranked_ids.index(case.expected_evidence_id) + 1 if case.expected_evidence_id in ranked_ids else None
    cited_ids = [item.evidence_id for item in result.response.evidence]
    return {
        ...
        "hit_at_k": int(case.expected_evidence_id in ranked_ids[:top_k]),
        "reciprocal_rank": 1.0 / expected_rank if expected_rank else 0.0,
        "evidence_coverage": int(case.expected_evidence_id in cited_ids),
        "citation_validity": int(set(cited_ids).issubset(set(ranked_ids))),
        "retrieval_latency_ms": result.retrieved.latency_ms,
        "generation_latency_ms": result.response.generation_latency_ms,
        "total_latency_ms": result.response.total_latency_ms,
...
def write_results(rows: list[dict[str, Any]]) -> None:
    ...
    summary = {
        "cases": len(rows),
        "rag_invocation_count": sum(row["rag_invoked"] for row in rows),
        ...
        "citation_validity": mean(row["citation_validity"] for row in rows),
        "mean_retrieval_latency_ms": mean(row["retrieval_latency_ms"] for row in rows),
        "mean_generation_latency_ms": mean(row["generation_latency_ms"] for row in rows),
        "mean_total_latency_ms": mean(row["total_latency_ms"] for row in rows),
        "mean_prompt_size_bytes": mean(row["prompt_size_bytes"] for row in rows),
        "mean_output_size_bytes": mean(row["output_size_bytes"] for row in rows),
        "evaluation_scope": "Hand-authored threat queries against the versioned corpus; not a CIC-IoT-2023 test-set result.",
    }
...
def main() -> None:
    ...
    rows = [evaluate_case(pipeline, case, top_k) for case in CASES]
    write_results(rows)
