...
DEFAULT_CORPUS_FILE = PROJECT_ROOT / "knowledge_base" / "cybersecurity_sources.json"
...
class KnowledgeBase:
    ...
    def __init__(self, records: Iterable[Mapping[str, Any]]) -> None:
        ...
        if not self._records:
            raise ValueError("knowledge corpus must contain at least one record")

    @classmethod
    def from_json(cls, corpus_file: Path = DEFAULT_CORPUS_FILE) -> "KnowledgeBase":
        with corpus_file.open("r", encoding="utf-8") as handle:
            records = json.load(handle)
        if not isinstance(records, list):
            raise ValueError("knowledge corpus JSON must contain a list")
        return cls(records)
    ...
    def search(
        ...
        query_tokens = self._query_tokens(request)
        ranked: list[tuple[float, Mapping[str, Any]]] = []

        for record in self._records:
            searchable_text = " ".join(
                str(record.get(field, ""))
                for field in ("document_title", "document_type", "content")
            )
            document_tokens = set(_TOKEN_PATTERN.findall(searchable_text.lower()))
        ...
        selected = tuple(
            self._to_evidence(record, score)
        ...
        return RetrievedEvidence(
            ...
            candidates_considered=len(self._records),
        )

    @staticmethod
    def _query_tokens(request: RAGRequest) -> set[str]:
        context_text = json.dumps(request.context, sort_keys=True, default=str)
        query_text = " ".join(
            (
                request.prediction,
        ...
        return set(_TOKEN_PATTERN.findall(query_text.lower()))
    ...
    def _to_evidence(
        ...
        return EvidenceReference(
            evidence_id=str(record["document_id"]),
