...
class DeterministicGroundedGenerator:
    ...
    def generate(
        ...
        first_title = (
            evidence[0].document_title
            if evidence
            else "selected guidance"
        )
        ...
        return {
            "threat": (
                f"Possible {request.prediction.lower()} activity"
            ),
            "detection_interpretation": (
            ...
            "uncertainty_reason": (
                ...
                "The edge result is not a confirmed diagnosis."
            ),
            "likely_attack_behavior": (
                f"The retrieved guidance most closely matches "
                f"{first_title}; this is an inference from limited "
                "structured context."
            ),
            ...
            "caveat": (
                "This offline baseline is evidence-grounded but not "
                "a substitute for analyst verification or a validated "
                "production LLM."
            ),
...
def _recommendation_for(
    ...
    return recommendations.get(
        evidence_id,
        "Review the cited guidance, monitor related activity, and "
        "apply least-privilege controls appropriate to the confirmed "
        "investigation.",
    )